//go:build lmstudio_live

// Package agent live LM Studio integration tests.
//
// These tests require a running LM Studio server on localhost:1234 and are
// excluded from the default build. Run them explicitly with:
//
//	go test -tags lmstudio_live -run TestLMStudioLive -v -timeout 120s ./internal/agent/
//
// Deterministic, always-on coverage for the OpenAI-compatible request/response
// path lives in chat_openai_lmstudio_test.go.
package agent
