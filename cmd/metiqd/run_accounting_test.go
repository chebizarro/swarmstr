...
func TestExecuteAgentRunWithFallbacks_PersistsFallbackState(t *testing.T) {
	...
	executeAgentRunWithFallbacks(context.Background(), runID, req, primary, []agent.Runtime{fallback}, []string{"claude-sonnet", "claude-haiku"}, nil, jobs)
...
func TestExecuteAgentRunWithFallbacks_ClearsFallbackStateOnPrimarySuccess(t *testing.T) {
	...
	executeAgentRunWithFallbacks(context.Background(), runID, req, primary, nil, []string{"claude-sonnet"}, nil, jobs)
