...
> **Round 14 (2026-07-26): xfny.5 final skills.proposals trio BUILT — epic xfny fully closed (all 22 WS-G skills methods).** Built `skills.proposals.historyStatus`/`historyScan`/`requestRevision` (`3114ae8`). historyStatus/historyScan = bounded, paged git-history scan of SKILL.md changes (optional direction/cursor, rename detection with `-M`). requestRevision RESOLVED the old human_input_required blocker: openclaw forwards to chat.send-with-suppressed-provenance for {runId,status}; metiq's native equivalent is the managed-run machinery — resolves the proposal, builds a revision prompt from instructions + staged draft, and `launchManagedRun`s a real agent run (same seam as talk.client.toolCall), returning {runId,status}. Idempotency: key reserved + `jobs.Begin` atomically under one mutex before publishing, so concurrent replays return the same runId (race-tested, 8 parallel → 1 launch). Git-scan hardened: exec arg-vector, hex-validated cursor, glob pathspec pinned to workspace, byte/entry/time caps. Oracle caught a P0 double-launch race + 3 P1s (unbounded idempotency map, unbounded git output, rename-dest-only), all fixed. **Gateway parity: 313 → 316 implemented, 8 missing.** The 8 remaining: `nuqy` (openclaw.setup ×5), `uhup` (controlUi ×2), `ko2f` (message.action ×1) — all product/infra-decision-gated.
>
> **Round 13 (2026-07-26): ngrd cooperative suspend/resume machinery BUILT.** Built a real suspend coordinator (`internal/gateway/suspend`, `bce329b`): durable suspension-id lifecycle `idle→preparing→suspended→resuming→idle` persisted to an atomic-JSON ledger (crash-recoverable, fail-closed — a daemon that crashed while suspended comes back gated until explicit resume), and wired `gateway.suspend.prepare/status/resume`. HONEST quiesce model: timer-driven workers (cron scheduler, dreaming/promotion job, memory-compaction) are GATED via a shared `AcceptingWork()` gate; interactive/event-driven work (agent runs, DM turns, task queue) is REPORTED-ONLY (drained not killed) and surfaced via `inFlight` + `scope:background-dispatch` + `interactiveActive` so the response never implies daemon-wide quiescence (optional full-quiescence admission-lease deferred → `bnwt`). Oracle caught a P0 startup-ordering race (worker closure read the coordinator global unsynchronized before its ledger path was known → fixed with `atomic.Pointer[Coordinator]`) and a P1 honesty gap. **Gateway parity: 310 → 313 implemented, 11 missing.** Reused the restart.* sibling precedent (restartCh scheduler + restart.preflight readiness inspector). The 11 remaining: `nuqy` (openclaw.setup ×5), `xfny.5` (skills.proposals ×3), `uhup` (controlUi ×2), `ko2f` (message.action ×1) — all product/infra-decision-gated.
>
> **Round 12 (2026-07-26): qmxu plugin UI-surface contribution model BUILT + 5p0v closed (product decision → option A).** Answered the 8 design questions and built it end-to-end (`2ce63d7`): (1) SDK Manifest v4 — optional `surfaces:{board_widgets,data_bindings,action_verbs,session_actions}` on both `manifest.Manifest` and `sdk.Manifest`, v3 back-compat; (2) host-side `internal/plugins/surface` registry aggregating contributions across goja+node with `plugin.surface.changed` push event + `GojaPluginManager.InvokeSurface` (dispatch into the owning plugin's sandboxed VM); (3) board unify (delivers **5p0v**) — `board.data.read`/`board.action` extend the core binding/verb allowlists via the registry, all grant-gated by `board.widget.grant`, plus the `canvas-doc` read-only content source; (4) wired `plugins.uiDescriptors`/`plugins.sessionAction`/`plugin.surface.refresh`. **Security model:** every plugin binding/verb/session-action is inert until the operator grants the declaring widget AND the registry resolves the id to a unique owning plugin; dispatch runs only the plugin's own sandboxed code; `sessionAction` derives `session_id` from the ticket's SessionKey (not caller-supplied); fail-closed throughout. **Oracle security review caught 2 real cross-plugin exploits pre-merge** — temporal id-hijacking (fixed: canonical dot-free plugin-id ownership, ids must be `<pluginID>.*`) and core-binding aliasing (fixed: reserved core ids injected from the authoritative allowlist); residual grant-vs-package-digest item filed as `ux1q`. **Gateway parity: 307 → 310 implemented, 14 missing.** The 14 remaining are all product/infra-decision-gated: `nuqy` (openclaw.setup ×5), `xfny.5` (skills.proposals ×3), `ngrd` (gateway.suspend ×3), `uhup` (controlUi ×2), `ko2f` (message.action).
>
