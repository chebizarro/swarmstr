...
- `metiq logs` shows cron tick events and job execution.
...
- Job not listed → add with `metiq cron add`.
...
- Relay write errors → check relay connectivity with `metiq channels status`.
...
```bash
metiq config get extra.heartbeat
metiq logs --lines 50
```
...
- No Nostr write → check relay config with `metiq channels status`.
