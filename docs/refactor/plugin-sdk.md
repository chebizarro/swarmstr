---
summary: "Planning doc: swarmstr Go plugin SDK for type-safe plugin development"
read_when:
...
This document captures planning thoughts for a formal Go plugin SDK that would make it easier to build type-safe swarmstr plugins without modifying core code.

## Problem

Currently, plugins require:
1. Forking or modifying `cmd/swarmstrd/main.go` to register tools
...
- **Discovery**: plugins auto-discovered from `~/.swarmstr/plugins/` like skills

## Proposed Interface

```go
// Plugin interface every swarmstr plugin must implement
...
p, err := plugin.Open("~/.swarmstr/plugins/my-plugin.so")
sym, err := p.Lookup("Plugin")
swarmstrPlugin := sym.(swarmstr.Plugin)
```
...
Use Go's Goja JS engine (already used in swarmstr for channel plugins):
...
For swarmstr's use case (Go-native, performance-sensitive), Option A (Go plugin .so) or a hybrid approach (Go for core tools, Goja for scripted tools) seems most appropriate.
