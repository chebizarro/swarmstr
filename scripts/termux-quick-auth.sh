...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/metiq/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "metiq Re-Auth" -c "After getting key, run: ssh $SERVER '~/metiq/scripts/mobile-reauth.sh'" --id metiq-auth
        ;;
    *)
