...
# Checks both Claude Code and metiq auth status
...
fetch_models_status_json() {
    metiq models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    metiq_status=$(check_metiq_auth 2>/dev/null || true)

    claude_expires=0
    metiq_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        metiq_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        metiq_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$METIQ_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$metiq_status" \
        --arg be "$metiq_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            metiq: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$metiq_status" == EXPIRED* ]] || [[ "$metiq_status" == MISSING* ]]; then
        echo "METIQ_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$metiq_status" == EXPIRING* ]]; then
        echo "METIQ_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "metiq Auth (~/.metiq/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'metiq doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active metiqd >/dev/null 2>&1; then
    echo -e "metiqd service: ${GREEN}running${NC}"
else
    echo -e "metiqd service: ${RED}NOT running${NC}"
fi
